...
const features = [
  { icon: Zap,          title: "Live Wholesale Prices",   description: "Tiered slab pricing auto-applies for verified kirana buyers." },
  { icon: PackageSearch,title: "Real-Time Catalogue",     description: "Redis-cached product discovery with performance built for scale." },
  { icon: ShieldCheck,  title: "Secure Authentication",   description: "JWT-based dual-role sign in for shop owners and wholesalers." },
  { icon: Truck,        title: "Automated Dispatch",      description: "Batch ordering workflows build the next shipment window." },
  { icon: Bell,         title: "Live Alerts",             description: "Notifications keep buyers and distributors synchronized." },
  { icon: BarChart3,    title: "Performance Insights",    description: "Operational data surfaced for inventory and orders." },
];
...
function ActionButton({ variant = "primary", children, title, ...props }) {
  const label = title ?? (typeof children === "string" ? children : "Action");
  return (
...
export default function LandingPage({ onGetStarted, onLogin }) {
  ...
  const marqueeItems = useMemo(() => [
    "JWT Authentication","Redis Caching","Atomic Stock Control","Socket.io Real-Time",
    "Dispatch Automation","MongoDB Aggregation","Role-Based Access","Batch Processing",
  ], []);
      ...
      <header className="landing-header">
        <div className="landing-header-inner">
          ...
          <div className="landing-actions">
            {hasLogin && <button type="button" className="landing-link" onClick={onLogin}>Log in</button>}
            <ActionButton onClick={onGetStarted}>Get Started <ArrowRight className="landing-icon" /></ActionButton>
          </div>
        </div>
      </header>
      ...
      <main className="landing-main">
        ...
        <section className="landing-hero">
          ...
          <div className="landing-hero-image-wrapper">
            <img src={workersImg} alt="Warehouse" className="landing-hero-image" loading="eager" />
            ...
            <div className="landing-hero-cards">
              <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.7 }} className="landing-info-card card-glow">
                <BarChart3 className="landing-card-icon" /><div><p>Redis Cache</p><strong>98.2% hit rate</strong></div>
              </motion.div>
              <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.85 }} className="landing-info-card card-glow secondary">
                <Truck className="landing-card-icon" /><div><p>Dispatch Window</p><strong>Next in 6 hours</strong></div>
              </motion.div>
              <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0 }} className="landing-info-card card-glow tertiary">
                <Bell className="landing-card-icon" /><div><p>Live Notifications</p><strong>Realtime updates</strong></div>
              </motion.div>
            </div>
          </div>
        </section>
          ...
          <div className="landing-marquee-track">
            {Array(3).fill(marqueeItems).flat().map((item, idx) => (
              <span key={`${item}-${idx}`} className="landing-marquee-item">
                <span className="landing-marquee-dot" />{item}
              </span>
            ))}
          </div>
        </div>
            ...
            <div className="landing-problem-grid">
              <motion.article initial="hidden" whileInView="visible" viewport={{ once: true }} custom={0} variants={fadeUp} className="landing-card landing-card-alt red">
                ...
                <div className="landing-card-body">
                  {["Phone calls and WhatsApp messages lost in noise","Manual inventory tracking leads to frequent stockouts","Unpredictable delivery and middlemen overhead","No real-time visibility into availability"].map(t => (
                    <div key={t} className="landing-list-item"><XCircle className="landing-list-icon" /><span>{t}</span></div>
                  ))}
                </div>
              </motion.article>
              <motion.article initial="hidden" whileInView="visible" viewport={{ once: true }} custom={1} variants={fadeUp} className="landing-card landing-card-alt green">
                <div className="landing-card-top">
                  <img src={onlineStoreImg} alt="Modern" className="landing-card-image" />
                  <div className="landing-card-badge green">The KiranaConnect Way</div>
                </div>
                <div className="landing-card-body">
                  {["Instant ordering directly from verified wholesalers","Atomic stock management with no oversells","Predictable 6-hour dispatch windows","Redis performance for catalogue speed"].map(t => (
                    <div key={t} className="landing-list-item"><CheckCircle className="landing-list-icon" /><span>{t}</span></div>
                  ))}
                </div>
              </motion.article>
            </div>
        ...
        <section id="features" className="landing-section dark-bg">
          <div className="landing-section-inner">
            ...
            <div className="landing-feature-grid">
              {features.map((f, i) => (
                <motion.div key={f.title} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i} variants={fadeUp} className="landing-feature-card">
        ...
        <section id="workflow" className="landing-section workflow-section">
          <div className="landing-section-inner">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="landing-section-heading workflow-heading">
              ...
              <h2 className="workflow-title">From app open to doorstep delivery — every step engineered for precision.</h2>
            ...
            <div className="workflow-layout">
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} custom={1} variants={fadeUp} className="workflow-visual">
                <img src={warehouseSupplyImg} alt="Wholesale dispatch" loading="lazy" />
              </motion.div>
              <div className="workflow-list">
                {workflowSteps.map((step, i) => (
                  <motion.article key={step.title} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i * 0.2} variants={fadeUp}
                    className="workflow-step" style={{ '--step-accent': step.accent }}>
            ...
            <div className="workflow-image-strip">
              {[{src:onlineStoreImg,label:"Digital ordering"},{src:handshakeImg,label:"Verified partners"},{src:mobileDeliveryImg,label:"Mobile delivery"}].map(i=>(
                <div key={i.label}><img src={i.src} alt={i.label} /><span>{i.label}</span></div>
              ))}
            </div>
          </div>
        </section>
          ...
          <div className="landing-section-inner">
            ...
            <div className="tech-grid">
              {[
                ...
                <motion.article key={item.title} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i} variants={fadeUp} className="tech-card">
        ...
        <section className="landing-cta">
          <img src={handshakeImg} alt="Partnership" />
        ...
        <section id="team" className="landing-section landing-team">
          <div className="landing-section-inner">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="landing-section-heading landing-team-heading">
            ...
            <div className="landing-team-grid">
              {[
                ...
                <motion.div key={m.role} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i*0.5} variants={fadeUp} className="landing-team-card">
      ...
      <footer className="landing-footer">
        <div className="landing-footer-inner">
          ...
          <div className="landing-footer-badges">
            <span>Built for MERN</span><span>Real-time Sync</span><span>Atomic Stock</span>
          </div>
        </div>
      </footer>
    </div>
